...
df_singleSVI_multiPar <- df_singleSVI_multiPar[df_singleSVI_multiPar$Question %in% c("beautiful", "green"),]
...
df_multiSVI_singlePar <- df_multiSVI_singlePar[df_multiSVI_singlePar$Question %in% c("beautiful", "green"),]
...
# separate the data for 'beautiful' and 'green' questions
df_beautiful <- df_par_from_svi_from %>% 
  filter(Question == "beautiful") %>%
...
corr_matrix <- df_combined %>%
  ...
  summarize(
    correlation = cor(Score_beautiful, Score_green, use = "complete.obs"),
    p_value = cor.test(Score_beautiful, Score_green, use = "complete.obs")$p.value,
    n = n(),
    .groups = "drop"
  ) %>%
...
# save figure ----
ggsave(
  "green-beautiful-matrix-paired-locations.png",
...
calculate_cubic_r2 <- function(data) {
  ...
  model <- lm(Score_beautiful ~ Score_green + I(Score_green^2) + I(Score_green^3), data = data)
...
p_location_scatter <- df_combined %>%
  ggplot(aes(x=Score_green, y=Score_beautiful)) +
  geom_point() +
  ...
  labs(
    ...
    y = expression(atop(bold("Participants from"), "Beautiful perception Q scores")),
    title = NULL)
p_location_scatter

ggsave(
  "green-beautiful-scatter-paired-locations.png",
